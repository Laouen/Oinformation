...
def smoothSoftPlus(X):
    return np.log1p(np.exp(X-3)) 
...
def generate_relu_sistem(alpha: float=1.0, beta: float=1.0, pow_factor: float=0.5, T: float=10000):
    ...
    X1 = alpha*np.power(smoothSoftPlus(Z_syn), pow_factor)    + beta*Z_red
    X2 = -alpha*np.power(smoothSoftPlus(-Z_syn), pow_factor)  + beta*Z_red

    # ensure col order
    return pd.DataFrame({'X1': X1, 'X2': X2, 'Z_syn': Z_syn, 'Z_red': Z_red})[['X1', 'X2', 'Z_syn', 'Z_red']]
